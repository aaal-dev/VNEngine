...
struct Entity {
	...
	inline const RGBA& getBaseColor() const { return baseColor; }
	...
	Entity() {}
	Entity(std::string name, vec3 position, vec3 size, vec3 origin, RGBA color) 
	: name(name), position(position), size(size), origin(origin), baseColor(color) {
